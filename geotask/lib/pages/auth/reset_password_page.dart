...
class _ResetPasswordPageState extends State<ResetPasswordPage> {
  ...
  Future<void> _sendCode() async {
    if (!isValidEmail(_emailCtrl.text.trim())) {
      showAppSnackBar(context, 'Introduz um email válido.');
      return;
    }
    ...
    try {
      ...
      if (mounted) {
        ...
        showAppSnackBar(context, 'Código enviado como notificação. Verifica as notificações da app.');
      }
    } catch (e) {
  ...
  Future<void> _confirmAndReset() async {
    ...
    try {
      ...
      if (!ok) {
        showAppSnackBar(context, 'Código inválido ou expirado. Pede um novo código.');
        return;
      }
      ...
      if (mounted) {
        showAppSnackBar(context, 'Password atualizada com sucesso. Faz login com a nova password.');
